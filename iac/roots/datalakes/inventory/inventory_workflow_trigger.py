...
def lambda_handler(event, context):
    """
    Lambda function triggered by S3 object creation in the inventory folder.
    ...
    try:
        ...
        logger.info(f"File uploaded: {s3_path}")
        
        # Skip if not a .gz file
        if not object_key.lower().endswith('.gz') and '.gz.' not in object_key.lower():
            logger.info(f"File {s3_path} is not a .gz file, skipping workflow trigger")
            return {
                'statusCode': 200,
                'body': json.dumps('Non-gz file detected, workflow not triggered')
            }
        
        # Check if this is in the right path pattern for inventory
        # Based on bucket.tf, the pattern should be: ${var.APP}-${var.ENV}-inventory-data-source-primary/InventoryConfig/data/
        if not '/InventoryConfig/data/' in object_key:
            logger.info(f"File {s3_path} is not in the inventory data path, skipping workflow trigger")
            return {
                'statusCode': 200,
                'body': json.dumps('File not in inventory data path, workflow not triggered')
            }
        ...
        logger.info(f"Updating crawler {crawler_name} to target: {s3_path}")
        try:
            ...
            logger.info(f"Successfully updated crawler target to: {s3_path}")
        except Exception as crawler_error:
            logger.error(f"Error updating crawler target: {str(crawler_error)}")
            raise crawler_error
